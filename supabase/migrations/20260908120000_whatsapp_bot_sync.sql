-- Migration: WhatsApp Bot & Site Synchronization
-- 1. Add origin column to public.appointments if not exists
ALTER TABLE public.appointments
ADD COLUMN IF NOT EXISTS origin text DEFAULT 'web';

-- 2. Add public.appointments to supabase_realtime publication idempotently
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
-- 3. Update public.submit_public_booking to accept p_origin
-- Drop old 7-argument signature to prevent ambiguous overload resolution
DROP FUNCTION IF EXISTS public.submit_public_booking(uuid, timestamptz, text, text, text, text, text);
...
GRANT EXECUTE ON FUNCTION public.submit_public_booking(uuid, timestamptz, text, text, text, text, text, text) TO anon, authenticated;
