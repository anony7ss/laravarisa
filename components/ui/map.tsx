...
function DefaultLoader() {
  return (
    <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 dark:bg-black/60 backdrop-blur-xs">
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const errorHandler = (e: any) => {
      // Gracefully handle non-fatal sprite/glyph network glitches
    };
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  return createPortal(
    <div className={cn("pointer-events-none rounded-md bg-neutral-900 text-white px-2.5 py-1 text-xs text-balance shadow-md animate-in fade-in-0 zoom-in-95 duration-200 ease-out", className)}>
      {children}
    </div>,
    container,
  );
...
function MarkerLabel({ children, className, position = "top" }: MarkerLabelProps) {
  ...
  return (
    <div className={cn("absolute left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-medium text-neutral-900 dark:text-neutral-100", positionClasses[position], className)}>
      {children}
    </div>
  );
}
